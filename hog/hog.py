...
def hog_gcd(x, y):
    ...
    for i in range(1, min(x, y) + 1):
        if x % i == 0 and y % i == 0:
            ans = i
    return ans if min(x, y) != 0 else max(x, y)
    # END PROBLEM 4
...
def fuzzy_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    # update_score = simple_update(num_rolls, player_score, opponent_score, six_sided)
    update_score = player_score + \
        take_turn(num_rolls, player_score, opponent_score, dice)
    return fuzzy_points(update_score)
    # END PROBLEM 4
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        player_score, opponent_score = (
            score0, score1) if who == 0 else (score1, score0)
        strategy = strategy0 if who == 0 else strategy1
        num_rolls = strategy(player_score, opponent_score)
        new_score = update(num_rolls, player_score, opponent_score, dice)
        if who == 0:
            score0 = new_score
        else:
            score1 = new_score
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def constant_roll(a, b):
        return n
    return constant_roll
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=50000):
...
def boar_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    return num_rolls if boar_brawl(score, opponent_score) < threshold else 0 
    # END PROBLEM 10
...
def fuzzy_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    return num_rolls if fuzzy_points(score + boar_brawl(score, opponent_score)) - score < threshold else 0
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return 6 if fuzzy_strategy(score, opponent_score) < 12 else 0
    # END PROBLEM 12
