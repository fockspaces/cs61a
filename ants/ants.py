...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            original_ant, new_ant = place.ant, self
            if original_ant.can_contain(new_ant):
                original_ant.store_ant(new_ant)
            elif new_ant.can_contain(original_ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while place:
            if place.bees and not place.is_hive and self.lower_bound <= distance and distance <= self.upper_bound:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # check if chewing_turns valid
        if self.turns_to_chew == 0:
            if self.place.bees:
            # pick a random bee in same place
                bee = random.choice(self.place.bees)
                # kill the bee
                bee.reduce_health(bee.health)
                # set chewing_turns to chewing_turns
                self.turns_to_chew = self.chewing_turns
        # decrease the chewing_turns
        else:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        target.slow_turns = 5
        if target.action.__name__ != 'slow_behavior':
            original_function = target.action
            def slow_behavior(gamestate):
                if target.slow_turns == 1:
                        target.action = original_function
                if gamestate.time % 2 == 0:
                    original_function(gamestate)
                target.slow_turns -= 1
            target.action = slow_behavior
            # END Problem EC 1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        destination = self.place.entrance if self.is_scared and self.back_away > 0 else self.place.exit
        self.back_away -= 1
        if self.blocked():
