...
def closer_city(lat, lon, city_a, city_b):
    ...
    diff_a, diff_b = distance(target, city_a), distance(target, city_b)
    return get_name(city_a) if diff_a < diff_b else get_name(city_b)
...
def dejavu(t, n):
    ...
    if label(t) == n and is_leaf(t):
        return True
    return True in [dejavu(b, n - label(t)) for b in branches(t)]
