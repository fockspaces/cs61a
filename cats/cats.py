...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    match_paras = [e for e in paragraphs if select(e)]
    return '' if k >= len(match_paras) else match_paras[k] 
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def wrap(s):
        for word in remove_punctuation(s).split(' '):
            # print('DEBUG:', word)
...
def accuracy(typed, source):
    ...
    if not typed_words or not source_words:
        return 100.0 if typed_words == source_words else 0.0
    for i in range(min(len(typed_words), len(source_words))):
        if source_words[i] == typed_words[i]:
            match_counts += 1
    return (match_counts / len(typed_words)) * 100 
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        if (typed, source) in cache:
            cached_result, cached_limit = cache[(typed, source)]
            if cached_limit >= limit:
                return cached_result

        result = diff_function(typed, source, limit)
        cache[(typed, source)] = (result, limit) if result <= limit else (limit + 1, limit)
        return result
...
def feline_fixes(typed, source, limit):
    ...
    def helper(typed, source, cur_diff):
        if cur_diff > limit:
            return cur_diff
        if not typed or not source:
            return cur_diff + len(typed + source)

        diff = 0 if typed[0] == source[0] else 1
        return helper(typed[1:], source[1:], cur_diff + diff)
    # print("DEBUG:", diff)
...
def minimum_mewtations(typed, source, limit):
    ...
    def helper(typed, source, cur_diff):
        ...
        if typed[0] == source[0]:
            return helper(typed[1:], source[1:], cur_diff)
        
        add = helper(typed, source[1:], cur_diff + 1)
        remove = helper(typed[1:], source, cur_diff + 1)
        substitute = helper(typed[1:], source[1:], cur_diff + 1)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for w1, w2 in zip(typed, prompt):
        if w1 != w2:
            break
        match_count += 1
    progress = match_count / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time_intervals = [[times[i + 1] - times[i] for i in range(len(times) - 1)] for times in times_per_player]
    return match(words, time_intervals)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        least_time, cur_winner = float("inf"), None
        for j in player_indices:
            cur_time = time(match, j, i)
            if cur_time < least_time :
                least_time, cur_winner = cur_time, j
        result[cur_winner].append(get_word(match, i))
    return result
    # END PROBLEM 10
