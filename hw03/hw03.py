...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def torque_balanced(side1, side2):
        return length(side1) * total_weight(end(side1)) == length(side2) * total_weight(end(side2))
    if is_planet(m):
        return True
    left_s, right_s = left(m), right(m)
    return torque_balanced(left_s, right_s) and balanced(end(left_s)) and balanced(end(right_s))
